...
def extinction_corrected_photometry(self):
    ...
    e_gi = np.sqrt(ps1_eg**2 + ps1_ei**2)
    e_gz = np.sqrt(ps1_eg**2 + ps1_ez**2)
    e_gy = np.sqrt(ps1_eg**2 + ps1_ey**2)
    e_ri = np.sqrt(ps1_er**2 + ps1_ei**2)
    e_rz = np.sqrt(ps1_er**2 + ps1_ez**2)
    e_ry = np.sqrt(ps1_er**2 + ps1_ey**2)
    e_iy = np.sqrt(ps1_ei**2 + ps1_ey**2)
    e_iz = np.sqrt(ps1_ei**2 + ps1_ez**2)
    e_zy = np.sqrt(ps1_ez**2 + ps1_ey**2)

    #extinction in ps1 filters taken from Tonry et.al. 2012
    ag = (self.ebv)*(3.613 - 0.0972*(ps1g - ps1i) + 0.01*(ps1g - ps1i)**2)
    ar = (self.ebv)*(2.585 - 0.0315*(ps1g - ps1i))
    ai = (self.ebv)*(1.908 - 0.0152*(ps1g - ps1i))
    az = (self.ebv)*(1.499 - 0.0023*(ps1g - ps1i))
